...
namespace Odom
{
    ...
    bool dispatch(const char *prop, const char *inst, const char *value)
    {
        ...
        if (strcmp(prop, "compass") == 0)
        {
            float deg;
            int quality;
            if (!parseCompass(value, &deg, &quality))
            {
                sendError("Odo", "compass", "*", "parse_err");
                return false;
            }
        ...
        if (strcmp(prop, "mode") == 0)
        {
            ...
            {
                odom_setActive(true);
                if (cfg_odom_period_ms == 0)
                    cfg_odom_period_ms = 50;
            }
