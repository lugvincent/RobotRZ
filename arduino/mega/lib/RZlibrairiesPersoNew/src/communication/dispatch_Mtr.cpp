...
namespace Mtr
{
    ...
    bool dispatch(const char *prop, const char *inst, const char *value)
    {
        ...
        if (urg_isActive())
        {
            // Autorisé en urgence : lecture et arrêt
            if (strcmp(prop, "read") == 0 || strcmp(prop, "act") == 0)
            {
                // on laisse passer
            }
            else
            ...
            {
                sendError("Mtr", prop, inst, "emergency_active");
                return false;
            }
        }
        ...
        if (strcmp(prop, "mode") == 0)
        {
            ...
            {
                mtr_setModeModern(true);
                sendInfo("Mtr", "mode", "*", "modern");
                return true;
            }
            if (strcmp(value, "legacy") == 0)
            {
                mtr_setModeModern(false);
                sendInfo("Mtr", "mode", "*", "legacy");
                return true;
            }
        ...
        if (strcmp(prop, "scale") == 0)
        {
            ...
            if (!tok)
            {
                sendError("Mtr", "scale", "*", "parse");
                return false;
            }
            cfg_mtr_inputScale = atoi(tok);
            tok = strtok(NULL, ",");
            if (tok)
                cfg_mtr_outputScale = atoi(tok);
