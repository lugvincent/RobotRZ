...
#include "../system/urg.h" // urg_isActive(), urg_handle(), urg_clear()
#include <string.h>
#include <stdlib.h>
...
bool parseVPIV(const char *raw, MessageVPIV &out)
{
    ...
    if (!nextToken(raw, buf, sizeof(buf), pos))
        return false;
    out.type = buf[0];

    // 2) module (var)
    if (!nextToken(raw, out.module, sizeof(out.module), pos))
        return false;

    // 3) prop
    if (!nextToken(raw, out.prop, sizeof(out.prop), pos))
        return false;

    // 4) inst
    if (!nextToken(raw, out.inst, sizeof(out.inst), pos))
        return false;
...
void communication_processInput()
{
    ...
    while (MAIN_SERIAL.available())
    {
        ...
        if (c == '#' && receiving)
        {
            ...
            if (parseVPIV(rxBuffer, msg))
            {
                ...
                if (urg_isActive())
                {
                    ...
                    else
                    {
                        ...
                        sendError("COM", "error", "A", "lock - STOP actif");
                    }
                }
                else
                {
            ...
            {
                // parsing invalide
                sendError("COM", "error", "A", "Parse Format invalide");
            }
        ...
        if (receiving)
        {
            ...
            else
            {
                ...
                sendError("COM", "error", "A", "Overflow");
            }
        }
    }
}
