...
/* --------------------------------------------------------------------
 * VPIV info avec anti-spam
 * ------------------------------------------------------------------*/
static void sendCtrlLInfo(const char *state)
{
    if (strcmp(last_vpiv_state, state) == 0)
        return;

    strncpy(last_vpiv_state, state, sizeof(last_vpiv_state) - 1);
    last_vpiv_state[sizeof(last_vpiv_state) - 1] = 0;

    sendInfo("ctrl", "L", "*", state);
}
...
void ctrl_L_update()
{
    ...
    int16_t angle = (int16_t)fs_lastAngle();
