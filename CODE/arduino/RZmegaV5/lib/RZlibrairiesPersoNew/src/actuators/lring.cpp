...
static LringExpression _exprFromName(const char* s) {
    if (!s) return LRING_EXPR_NEUTRE;
    if (strcasecmp(s,"neutre")==0) return LRING_EXPR_NEUTRE;
    if (strcasecmp(s,"voyant")==0) return LRING_EXPR_VOYANT;
    if (strcasecmp(s,"eclairage")==0) return LRING_EXPR_ECLAIRAGE;
    if (strcasecmp(s,"sourire")==0) return LRING_EXPR_SOURIRE;
    if (strcasecmp(s,"triste")==0) return LRING_EXPR_TRISTE;
    if (strcasecmp(s,"alerte")==0 || strcasecmp(s,"urgence")==0) return LRING_EXPR_URGENCE;
    return LRING_EXPR_NEUTRE;
}

static const char* _nameFromExpr(LringExpression e) {
    switch(e) {
        case LRING_EXPR_NEUTRE: return "neutre";
        case LRING_EXPR_VOYANT: return "voyant";
        case LRING_EXPR_ECLAIRAGE: return "eclairage";
        case LRING_EXPR_SOURIRE: return "sourire";
        case LRING_EXPR_TRISTE: return "triste";
        case LRING_EXPR_URGENCE: return "alerte";
...
static void _applySourire() {
    lringhw_clear();
    int idx[] = {4,5,6,7,8};
    for (int i=0;i<5;i++) lringhw_setPixel(idx[i], 0,160,0);
    lringhw_show();
}
...
static void _applyTriste() {
    lringhw_clear();
    int idx[] = {10,11,0,1,2};
    for (int i=0;i<5;i++) lringhw_setPixel(idx[i], 200,100,0);
    lringhw_show();
}
...
void lring_setExternMode(bool enable) {
    if (exprForcedByUrg) return; // urgence interdit tout changement
    modeExtern = enable;
}
...
void lring_applyExpression(const char* exprName, unsigned long timeoutMs) {
    ...
    if (exprForcedByUrg) return; // urgence = override total
    ...
    switch(curExpr) {
        case LRING_EXPR_NEUTRE:   _applyNeutral(); break;
        case LRING_EXPR_ECLAIRAGE:_applyEclairage(); break;
        case LRING_EXPR_SOURIRE:  _applySourire(); break;
        case LRING_EXPR_TRISTE:   _applyTriste(); break;
        case LRING_EXPR_VOYANT:   _applyVoyant_on(); break;
        case LRING_EXPR_URGENCE:  _applyUrgence_on_full(); break;
    }
...
void lring_processPeriodic() {
    ...
    if (exprForcedByUrg || curExpr == LRING_EXPR_URGENCE) {
        ...
        if (now - lastBlinkToggle >= 500) {
            ...
            if (blinkState) {
                if (longueUrg) _applyUrgence_on_single();
                else           _applyUrgence_on_full();
            } else {
                if (longueUrg) _applyUrgence_off_single();
                else           _applyUrgence_off_full();
            }
        }
        return;
    }
    ...
    if (curExpr == LRING_EXPR_VOYANT) {
        if (now - lastBlinkToggle >= 1000) {
            blinkState = !blinkState;
            lastBlinkToggle = now;
            if (blinkState) _applyVoyant_on();
            else            _applyVoyant_off();
        }
    }
}
