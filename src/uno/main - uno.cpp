/*********************************************************************
 * Code de RZunoV6.ino  actualisé pour RZ version sans RPI et reprise dans main.cpp
 *
...
// Définitions pour clarté
#define MEGA_SERIAL Serial  // Serial hardware de l'Uno
#define DEBUG_SERIAL Serial // Même port pour debug (attention aux conflits)
...
static bool parseBufferAndStore(const char *s)
{
  ...
  if (*p1 == '-' || isdigit((unsigned char)*p1))
  {
    ...
    if (strchr(p1, '-') || strlen(p1) > 1 || atoi(p1) > 9)
      isModern = true;
  }
...
void setup()
{
  // Serial hardware pour communication avec Mega ET debug PC
  // NOTE: Ne pas utiliser Serial.print() pendant la communication normale
  // ou gérer avec un flag de debug
  MEGA_SERIAL.begin(SERIAL_BAUD);

  // Pour debug, on peut envoyer un message puis désactiver
  MEGA_SERIAL.println(F("UNO moteurs boot OK - Attente Mega..."));
  delay(100); // Laisse le temps d'envoyer
...
void loop()
{
  ...
  if (!hasRecentCmd())
  {
    ...
    int step = 20;
    if (curL > 0)
      curL = max(0, curL - step);
    else if (curL < 0)
      curL = min(0, curL + step);

    if (curR > 0)
      curR = max(0, curR - step);
    else if (curR < 0)
      curR = min(0, curR + step);
