...
void mic_handleReadList(const int* indices, int n, const char* type) {
    ...
    if (n == 0) {
        for (int i = 0; i < MIC_COUNT; ++i) {
            if (strcmp(t,"raw")==0) send_val("raw", i, mic_lastRaw[i]);
            else if (strcmp(t,"peak")==0) send_val("peak", i, mic_lastPeak[i]);
            else /*rms*/ send_val("rms", i, mic_lastRMS[i]);
        }
        return;
    }
    ...
    for (int k = 0; k < n; ++k) {
        int idx = indices[k];
        if (idx < 0 || idx >= MIC_COUNT) continue;
        if (strcmp(t,"raw")==0) send_val("raw", idx, mic_lastRaw[idx]);
        else if (strcmp(t,"peak")==0) send_val("peak", idx, mic_lastPeak[idx]);
        else send_val("rms", idx, mic_lastRMS[idx]);
    }
}
...
void mic_setThreshold(const int* indices, int n, int th) {
    if (n == 0) {
        for (int i=0;i<MIC_COUNT;i++) cfg_mic_threshold[i] = th;
    } else {
        for (int k=0;k<n;k++) {
            int idx = indices[k];
...
void mic_setWin(unsigned long ms)  { if (ms<5) ms=5; cfg_mic_win_ms = ms; }
...
void mic_processPeriodic() {
    ...
    if (now - lastPublish < cfg_mic_freq_ms) return;
    lastPublish = now;
